...
describe("POST /users", function () {
  test("works for users: create non-admin, as admin", async function () {
  ...
  test("works for users: create admin, as admin", async function () {
    const resp = await request(app)
        .post("/users")
        .send({
          username: "u-new",
...
describe("PATCH /users/:username", () => {
  ...
  test("bad request if invalid data, as same user", async function () {
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
          firstName: 42,
  ...
  test("works: set new password, as same user", async function () {
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
